...
# test_bilibili.py 在 test/ 目录，往上一级就是项目根目录
...
TASK_CASES = [
    {
        "name": "测例1：搜索\"agent是什么\"并播放第一个视频",
        "description": (
            "请完成以下任务：\n"
            "1. 搜索B站（Bilibili）\n"
            "2. 从搜索结果中进入B站首页\n"
            "3. 点击B站页面上的搜索栏，搜索关键词'agent是什么'\n"
            "4. 在搜索结果中点击播放第一个视频\n"
            "5. 完成后调用 done 工具结束任务\n"
        ),
    },
    {
        "name": "测例2：搜索\"Java面试\"并返回3个播放量最高的视频链接",
        "description": (
            "请完成以下任务：\n"
            "1. 直接访问 https://www.bilibili.com 进入B站首页\n"
            "2. 点击B站页面上的搜索栏，搜索关键词'Java面试'\n"
            "3. 在搜索结果页面，将排序方式改为'按播放量排序'\n"
            "4. 在搜索结果页面，读取播放量最高的前3个视频及其完整信息，包括：\n"
            "   - 视频名\n"
            "   - 播放量\n"
            "   - 发布日期\n"
            "   - up主\n"
            "   - 视频链接\n"
            "5. 完成后调用 done 工具结束任务，在done的result中按以下格式列出这3个视频：\n"
            "\n"
            "格式：\n"
            "1. 【视频名】—— 播放量：XXX万 —— 发布日期：YYYY-MM-DD —— up主：XXX\n"
            "   链接：https://www.bilibili.com/video/BVxxxxxx/\n"
            "2. 【视频名】—— 播放量：XXX万 —— 发布日期：YYYY-MM-DD —— up主：XXX\n"
            "   链接：https://www.bilibili.com/video/BVxxxxxx/\n"
            "3. 【视频名】—— 播放量：XXX万 —— 发布日期：YYYY-MM-DD —— up主：XXX\n"
            "   链接：https://www.bilibili.com/video/BVxxxxxx/\n"
            "\n"
            "重要提示：\n"
    ...
    {
        "name": "测例3：搜索\"小约翰可汗\"并返回该 up 主的3个最新视频链接",
        "description": (
            "请完成以下任务：\n"
            "1. 直接访问 https://www.bilibili.com 进入B站首页\n"
            "2. 点击B站页面上的搜索栏，搜索关键词'小约翰可汗'\n"
            "3. 在搜索结果中找到up主'小约翰可汗'的主页链接，点击进入其主页\n"
            "4. 在up主主页，读取最新发布的前3个视频及其完整信息，包括：\n"
            "   - 视频名\n"
            "   - 播放量\n"
            "   - 发布日期\n"
            "   - 视频链接\n"
            ...
            "\n"
            "格式：\n"
            "1. 【视频名】—— 播放量：XXX万 —— 发布日期：YYYY-MM-DD\n"
            "   链接：https://www.bilibili.com/video/BVxxxxxx/\n"
            "2. 【视频名】—— 播放量：XXX万 —— 发布日期：YYYY-MM-DD\n"
            "   链接：https://www.bilibili.com/video/BVxxxxxx/\n"
            "3. 【视频名】—— 播放量：XXX万 —— 发布日期：YYYY-MM-DD\n"
            "   链接：https://www.bilibili.com/video/BVxxxxxx/\n"
            "\n"
            "重要提示：\n"
            "- visual_analyze 只能识别元素位置坐标，无法从截图中读取文本信息，且坐标点击不可靠。**整个任务中禁止调用 visual_analyze。**\n"
...
async def run_batch_test(case_indices: list[int] | None = None):
    ...
    try:
        ...
        with open("data/test_results_bilibili.txt", "w", encoding="utf-8") as f:
            f.write(f"B站测试结果 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        ...
        print(f"📄 详细结果已保存到 data/test_results_bilibili.txt")
    except Exception as e:
